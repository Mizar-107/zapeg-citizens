...
For normal provider responses, budget the Minecraft mod's `CITIZENS_BRAIN_REQUEST_TIMEOUT_MS` above the sidecar's worst case. For durable-job requests that is `CITIZENS_MAX_JOB_REQUEST_SECONDS` (100, a planning-loop *entry* bound) plus one final pass of queue wait (20) plus provider timeout (90) ≈ 210 seconds; the mod default is 300 seconds (`test_config` guards the sum). This reduces ordinary queued-request timeouts but is not a hard total deadline because Python's provider timeout applies to socket operations. The mod also defaults `CITIZENS_BRAIN_TURN_TIMEOUT_MS` to 600,000 ms for ordinary dialogue. Durable physical jobs use their persisted action, model-call, and active-time budgets instead.
